...
func (a *API) GetNormalTransactions(req GetTransactionsRequest) ([]Transaction, error) {
	...
	call := fmt.Sprintf("https://api.etherscan.io/api?module=account&action=txlist&address=%s&startblock=%d&endblock=%d&page=%d&offset=%d&sort=%s&apikey=%s", req.Address, req.StartBlock, req.EndBlock, req.Page, req.Offset, sort, a.apiKey)
	resp, err := http.Get(call)
	if err != nil {
		return nil, err
	}
...
func (a *API) GetInternalTransactions(req GetTransactionsRequest) ([]Transaction, error) {
	...
	call := fmt.Sprintf("https://api.etherscan.io/api?module=account&action=txlistinternal&address=%s&startblock=%d&endblock=%d&page=%d&offset=%d&sort=%s&apikey=%s", req.Address, req.StartBlock, req.EndBlock, req.Page, req.Offset, sort, a.apiKey)
	resp, err := http.Get(call)
	if err != nil {
		return nil, err
	}
